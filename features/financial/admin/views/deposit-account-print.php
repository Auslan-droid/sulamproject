<?php
/**
 * Deposit Account Print Template
 */
...
<head>
    <meta charset="UTF-8">
    <title>Akaun Terimaan - Print</title>
    <style>
        @page {
            size: A4 landscape;
            margin: 0;
        ...
        .col-receipt { width: 80px; text-align: center; }
    ...
    <table class="print-table">
        <thead>
            <tr>
                ...
                <th class="col-receipt">No. Resit</th>
                <th class="col-from">Terima Dari</th>
        ...
        <tbody>
            <?php if (empty($deposits)): ?>
                <tr>
                    <td colspan="<?php echo 6 + count($categoryColumns) + 1; ?>" style="text-align: center; padding: 20px;">
                        Tiada rekod.
                    </td>
                </tr>
            <?php else: ?>
                ...
                ?>
                    <tr>
                        <td class="col-date"><?php echo date('d/m/Y', strtotime($deposit['tx_date'])); ?></td>
                        <td class="col-receipt"><?php echo htmlspecialchars($deposit['receipt_number'] ?? '-'); ?></td>
                        <td class="col-from"><?php echo htmlspecialchars($deposit['received_from'] ?? ''); ?></td>
                        <td class="col-desc"><?php echo htmlspecialchars($deposit['description'] ?? ''); ?></td>
                        <td class="col-method"><?php echo strtoupper($deposit['payment_method'] ?? 'cash'); ?></td>
                        <td class="col-ref"><?php echo htmlspecialchars($deposit['payment_reference'] ?? '-'); ?></td>
                        <?php foreach ($categoryColumns as $col): ?>
                            <td class="col-amount">
                                <?php 
                                $amt = (float)($deposit[$col] ?? 0);
                ...
                <!-- Totals Row -->
                <tr class="footer-row">
                    <td colspan="6" style="text-align: right; padding-right: 10px;">JUMLAH:</td>
                    <?php foreach ($categoryColumns as $col): ?>
